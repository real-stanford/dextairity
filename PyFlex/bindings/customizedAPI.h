...
pair<int, int> pyflex_add_cloth_square(py::array_t<float> position, py::array_t<float> orientation, py::array_t<int> dimension, py::array_t<float> stiffness, float mass, bool center)
{
    // radius
...
pair<int, int> pyflex_emit_particles_cone(py::array_t<float> position, py::array_t<float> orientation, int num_layer, float alpha, float velocity, float mass, int input_baseIndex)
{
    // position: float (initX, initY, initZ)
    auto ptr_position = (float *)position.request().ptr;
    Point3 p = Point3(ptr_position[0], ptr_position[1], ptr_position[2]);
