...
class WSG50(Gripper):
    ...
    def __init__(self, tcp_ip, tcp_port=1000):
        self.tcp_ip = tcp_ip
    ...
    def open(self, blocking=True, ** kwargs):
        self.move(50, blocking=blocking, **kwargs)
    ...
    def move(self, position, speed=300, blocking=True):
        """
    ...
    def grip(self, force=80, blocking=True):
        MESSAGE = str.encode(f"GRIP({force})\n")
        self.tcp_sock.send(MESSAGE)
        if blocking:
            return self.wait_for_msg(b"FIN GRIP\n")

    def release(self, part_width=10, speed=300, blocking=True):
        """
