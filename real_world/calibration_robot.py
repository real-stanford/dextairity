...
from realur5 import UR5, UR5RTDE
...
if __name__ == "__main__":
    cam = cameras.KinectClient('128.59.23.32', '8080')

    j_vel = 1
    j_acc = 0.5

    blow_ur5 = UR5(tcp_ip='192.168.0.142', tcp_port=30002, rtc_port=30003, velocity=j_vel, acceleration=j_acc)
    left_ur5 = UR5RTDE('192.168.0.139')
    right_ur5 = UR5RTDE('192.168.0.204')
    ...
    np.savetxt('cam_pose/cam2blow_pose.txt',
                calibrate(cam, blow_ur5, 
    ...
    np.savetxt('cam_pose/cam2left_pose.txt',
    ...
    np.savetxt('cam_pose/cam2right_pose.txt',
                calibrate(cam, right_ur5, 
                    workspace_bounds = np.array([
                        [-0.7, -0.4],
                        [-0.4, -0.1],
                        [0.3, 0.3]
                    ]), ee_tip_offset=[0,0,0], calib_grid_step=0.15),
                delimiter=' ')
